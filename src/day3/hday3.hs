...
simulateMovement :: ([(Int, Int)], (Int, Int)) -> [Char] ->  ([(Int, Int)], (Int, Int))
simulateMovement (visited, (curX, curY)) (dir:nString)
  | dir == 'R' = (visited ++ [ (x,y)  | x<-[curX..curX+n], y<-[curY] ], (curX+n, curY))
  | dir == 'L' = (visited ++ [ (x,y)  | x<-[curX-n..curX], y<-[curY] ], (curX-n, curY))
  | dir == 'U' = (visited ++ [ (x,y)  | x<-[curX], y<-[curY..curY+n] ], (curX, curY+n))
  | dir == 'D' = (visited ++ [ (x,y)  | x<-[curX], y<-[curY-n..curY] ], (curX, curY-n))
 where n = read nString

absoluteSum = (\x y -> (abs x) + (abs y))

findShortestFromOrigin :: [(Int, Int)] -> [(Int, Int)] -> Int
findShortestFromOrigin visited1 visited2=
...
main = do
  ...
  let firstWireVisited = fst $ foldl simulateMovement ([], (0,0)) firstWire
  let secondWireVisited = fst $ foldl simulateMovement ([], (0,0)) secondWire
  print $ findShortestFromOrigin firstWireVisited secondWireVisited
