...
simulateMovement (visited) (dir:nString)
  | dir == 'R' = visited ++ [ (x,y)  | x<-[curX..curX+n], y<-[curY] ]
  | dir == 'L' = visited ++ (reverse [ (x,y)  | x<-[curX-n..curX], y<-[curY] ])
  | dir == 'U' = visited ++ [ (x,y)  | x<-[curX], y<-[curY..curY+n] ]
  | dir == 'D' = visited ++ (reverse [ (x,y)  | x<-[curX], y<-[curY-n..curY] ])
 where
  n = read nString
...
findShortestDistanceFromOrigin visited1 visited2=
  foldr1 min $ map (uncurry absoluteSum) $ tail $ intersect visited1 visited2

-- findShortestWireFromOrigin :: [(Int, Int)] -> [(Int, Int)] -> Int
-- findShortestWireFromOrigin visited1 visited2=
--   foldr1 min $ map (uncurry absoluteSum) $ tail $ intersect visited1 visited2
